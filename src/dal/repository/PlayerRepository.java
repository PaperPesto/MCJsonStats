...
public class PlayerRepository extends AbstractRepository implements IPlayerRepository{
	...
	public PlayerRepository(MyConfiguration config) {
		db = MongoClientConnection.getInstance(config).getDatabase(config.nameDB);	// Setto la stringa di conn. da configurazione
	...
	public List<PlayerDTO> getAllPlayers() {
		...
		while (cursor.hasNext()) {
			...
			player.minecraft_uuid = document.getString("minecraft_id");
	...
	public PlayerDTO getPlayerByNome(String nome, String cognome) {
		...
		Document document = coll.find( 
				new Document("nome", nome).append("cognome", cognome) 
				).first();
		...
		playerdto.minecraft_uuid = document.getString("minecraft_id");
	...
	public PlayerDTO getPlayerByNickname(String nickname) {
		Document document = coll.find( 
				new Document("nickanem", nickname) 
				).first();
